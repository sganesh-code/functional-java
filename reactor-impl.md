...
Add Reactor Core support to functional-java in the same artifact, while keeping the core functional primitives free of direct Reactor method pollution. Public application APIs should continue to use Task,
TaskEither, ReaderTaskEither, Resource, Deferred, Fiber, Outcome, and AsyncStream. Reactor types should only appear in a dedicated utility bridge package for framework edges such as Spring WebFlux.

Before implementation begins, create epic-planning.md with this plan and treat it as the execution source of truth.

## Public API Direction

- Keep Task, TaskEither, ReaderTaskEither, and AsyncStream as the preferred public abstractions.
- Do not add toMono, fromMono, toFlux, or fromFlux methods directly to functional primitives.
- Add a dedicated exported bridge package, for example io.github.senthilganeshs.fj.reactor, with utility classes such as:
    - ReactorTaskInterop
    - ReactorTaskEitherInterop
    - ReactorStreamInterop
- Reactor-specific signatures are allowed only in that bridge package.
- Core packages such as fj.ds and fj.stream must not depend on Reactor imports.
- Same artifact means build.gradle and module-info.java will need Reactor dependencies and module requirements, but Reactor usage must remain isolated by package convention.
...
- epic-planning.md contains this finalized approach.
...
4. Add Task.recover(...), Task.recoverWith(...), and Task.mapError(...)-style recovery helpers if needed by bridge implementation.
...
- The bridge package is clearly marked as interop/boundary support.
